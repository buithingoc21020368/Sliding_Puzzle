...
bool Graphics::setup()
{
    ...
    {
        std::string str = "assets/cat (" + std::to_string(i + 1) + ").png";
        char *cstr = new char[str.length() + 1];
        strcpy(cstr, str.c_str());

        cat[i + 1] = IMG_LoadTexture(renderer, cstr);
        if (cat[i] == nullptr)
        {
            std::cerr << str << '\n';
    ...
    for (int i = 0; i < t.size(); ++i)
    {
        ...
        if (drawnumber)
        {
            if (t[i].tileType() != Tile::type::invisible)
            {
                ...
                // std::cout << temp.x << "_" << temp.y << "_" << temp.w << " " << temp.h << '\n';
...
void Graphics::renderCat(const std::vector<Tile> &tiles, const std::vector<SDL_Rect> &positions)
{
    ...
    for (int i = tiles.size() - 2; i >= 0; --i)
    {
        for (int j = tiles.size() - 2; j >= 0; --j)
        {
            ...
            renderTexture(cat[id], tiles[j].position().x, tiles[j].position().y, &temp);
        }
    }
}
...
void Graphics::menuText(std::vector<Tile> tiles, const bool &catmode)
{
    ...
    SDL_Color tilecolour{237, 229, 239, 255};
    menutext = renderText("Cat Mode", "assets/Calibrib.ttf", fontcolour, 20);
    SDL_QueryTexture(menutext, NULL, NULL, &W, &H);
